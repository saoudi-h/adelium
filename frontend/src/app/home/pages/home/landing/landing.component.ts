...
@Component({
    ...
    template: `<section class="w-full">
        <div
            class="hero min-h-[calc(100vh-4rem)]"
            style="background-image: url('/assets/images/classroom_scene.png')">
            <div
                class="hero-overlay bg-black bg-opacity-0 dark:bg-opacity-20"></div>
            <div
                class="m-4 rounded-3xl bg-base-200/50 p-6 px-8 text-center text-neutral-content shadow-2xl shadow-black backdrop-blur-md first-line:hero-content">
                <div
                    class="relative flex max-w-4xl flex-col items-center"
                    @landingAnimation>
                    <h1
                        class="hero-item bg-gradient-to-r from-base-content via-primary to-base-content bg-clip-text font-sans text-res-big font-black text-transparent">
                        Découvrez un Univers d'Apprentissage Informatique
                    </h1>
                    <p
                        class="hero-item md:text-md xl:text-1xl mt-4 font-sans font-light text-base-content md:text-lg xl:text-xl">
                        Bienvenue sur la plateforme Adelium, votre passerelle
                        vers l'apprentissage informatique de qualité. Explorez
                        des formations, des ressources gratuites, des quiz, et
                        bien plus encore. Que vous soyez un débutant ou un
                        professionnel, nous avons tout ce dont vous avez besoin
                        pour développer vos compétences. Rejoignez-nous dès
                        maintenant et préparez-vous à élargir vos horizons
                        informatiques.
                    </p>
                    <div class="mt-8 flex flex-wrap justify-center gap-4">
                        <!-- formation btn  -->
                        <button
                            class="hero-item btn btn-primary btn-lg font-normal normal-case">
                            <svg
                        ...
                        <!-- presentation btn  -->
                        <button
                            class="hero-item btn btn-accent btn-lg font-normal normal-case">
                            <svg
                                xmlns="http://www.w3.org/2000/svg"
                                fill="none"
                                viewBox="0 0 24 24"
                    ...
                    <div class="hero-item mt-14 px-4 text-base-content md:px-8">
                        <p class="font-semibol text-center text-sm">
                    ...
                    <!-- shadow roounded primary -->
                    <div
                        class="pointer-events-none absolute z-0 aspect-square w-2/3 -translate-y-1/2 rounded-full bg-accent opacity-30 blur-3xl [transform:translate3d(0,0,0)]"></div>
                </div>
            </div>
        </div>
        <!-- shadow roounded primary -->
        <div
            class="pointer-events-none absolute right-0 top-1/2 z-0 aspect-square w-1/2 -translate-y-1/2 rounded-full bg-primary opacity-10 blur-3xl [transform:translate3d(0,0,0)]"></div>
    </section> `,
    animations: [
        trigger('landingAnimation', [
            transition(':enter', [
                // Lorsque le composant entre dans le DOM
                query(
                    '.hero-item',
                    style({
                        opacity: 0,
                        transform: 'rotateX(90deg) translate(-2rem,-2rem)',
                    })
                ), // Style initial pour les éléments
                query(
                    '.hero-item',
                    stagger('400ms', [
                        animate(
                            '0.4s cubic-bezier(.17,.67,.83,.67)',
                            style({
                                opacity: 1,
